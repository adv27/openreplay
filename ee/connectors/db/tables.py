...
def create_tables_clickhouse(db):
    with open(base_path / 'sql' / 'clickhouse_events.sql') as f:
        q = f.read()
    db.engine.execute(q)
    print('`connector_user_events` table created succesfully.')

    with open(base_path / 'sql' / 'clickhouse_events_buffer.sql') as f:
        q = f.read()
    db.engine.execute(q)
    print('`connector_user_events_buffer` table created succesfully.')

    with open(base_path / 'sql' / 'clickhouse_sessions.sql') as f:
        q = f.read()
    db.engine.execute(q)
    print('`connector_sessions` table created succesfully.')

    with open(base_path / 'sql' / 'clickhouse_sessions_buffer.sql') as f:
        q = f.read()
    db.engine.execute(q)
    print('`connector_sessions_buffer` table created succesfully.')
